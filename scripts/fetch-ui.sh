...
# every site in the family at once, with nothing in between to catch it.
set -euo pipefail
...
# Copying only on a version change would leave a cleaned public/ with no fonts
# and no error -- the page would silently fall back to system faces.
mkdir -p public
rm -rf public/fonts
